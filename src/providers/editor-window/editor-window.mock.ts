export class EditorWindowMock {
  ...
  tabInit(tabID: string){
    ...
    newTab.scrollIntoView({behavior: "instant", block: "end", inline: "end"});
...
//active method
activateTab(tabObject: any, targetID: any){
  let targetTabID: string = targetID;
  let targetTab: any = tabObject;
...
//close method
closeTab(tabObject: any, parentTab:any){
  // event.preventDefault();
  // event.stopPropagation();

let targetTab: any = tabObject;
let targetTabParentList: any = parentTab;
...
var e = document.body;
e.parentNode.removeChild(targetTab);
// targetTabParentList.removeChild(targetTab);
for(let b: number = 0; b < targetBodyList.length; b++){
  if(targetBodyList[b].id == targetTab.id){
    targetBody.removeChild(targetBodyList[b]);
